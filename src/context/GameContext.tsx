...
import { calculateAndUpdateElo, type EloMap, type GameModeKey, type EloPlayerSnapshot } from '../components/EloPlayer';
  ...
  return {
    ...
    case 'START_NEW_GAME':
      return {
        ...
        settings: {
      ...state.settings,
      matchId: state.settings.isTournament
    ? state.settings.matchId : `${Date.now()}-${Math.random().toString(36).substring(2, 10)}`,
    },
      };
...
const nextDealer = () => {
  ...
  setScreenHistory((prevHistory) => {
    ...
    if (currentScreen === 'elo') {
      const previous = prevHistory[history.length] || 'home';
      setCurrentScreen(previous)
      return prevHistory
    }
...
const getTimeFrameUserRankings = async (
  ...
  // 3. Charger les 200 derniers matchs
const { data: matches, error } = await supabase
  .from('match_history')
